...
test('release allowlist contains release docs, manifest.json, icons and product src files only', async () => {
  ...
  assert.ok(files.every(file => file === 'README.txt' || file === `CHANGES-${RELEASE_VERSION}.txt` || file === `QA-${RELEASE_VERSION}.txt` || file === 'manifest.json' || file.startsWith('icons/') || file.startsWith('src/')));
...
test('release ZIP is byte-for-byte reproducible and has one canonical root folder', async t => {
  ...
  assert.deepEqual(unpackedRootEntries, [`CHANGES-${RELEASE_VERSION}.txt`, `QA-${RELEASE_VERSION}.txt`, 'README.txt', 'icons', 'manifest.json', 'src'].sort());
