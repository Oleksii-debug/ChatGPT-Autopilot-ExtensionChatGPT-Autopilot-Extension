...
test('startup clears every legacy profile send lease because per-session durable operations own recovery', () => {
  ...
  state.sendArbiter.profileNextAllowedSendAt = 99_999;
  reconcileStateForStartup(state, 10_000);
  assert.equal(state.sendArbiter.lease, null);
...
test('legacy arbiter compatibility no longer blocks independent sessions or creates a post-send gap', () => {
  const state = createEmptyState(0);
  assert.equal(acquireSendLease(state, { sessionId: 's1', operationId: 'a', now: 1_000, ttlMs: 5_000 }), true);
  assert.equal(acquireSendLease(state, { sessionId: 's2', operationId: 'b', now: 1_001, ttlMs: 5_000 }), true);
  assert.equal(state.sendArbiter.lease.ownerSessionId, 's2');
  assert.equal(releaseSendLease(state, { sessionId: 's2', operationId: 'b' }), true);
  assert.equal(state.sendArbiter.profileNextAllowedSendAt, 0);
  assert.equal(acquireSendLease(state, { sessionId: 's1', operationId: 'c', now: 1_002, ttlMs: 5_000 }), true);
});
