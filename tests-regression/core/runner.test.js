...
test('submit effect observes durable SUBMITTING checkpoint without a profile-wide lease', async () => {
  ...
  const result = await coordinator.submitWithDurableCheckpoint({
    ...
    submit: async () => {
      ...
      assert.equal(persisted.sendArbiter.lease, null);
...
test('submit exception becomes ambiguous recovery without cooldown and without a profile-wide lease', async () => {
  const repo = new FakeRepository(fixture());
  const clock = { value: 0 };
  const coordinator = new DurableSubmissionCoordinator(repo, { now: () => clock.value, cryptoApi: webcrypto });
  const identity = await prepareForSubmit(coordinator, clock);
  ...
  assert.equal(
    ...
    'Надсилання не підтверджено. Автоматичне відновлення триває. Код: SUBMIT_EFFECT_EXCEPTION.',
  );
...
test('managed fresh-root uncertain submit durably binds observed conversation for exact recovery', async () => {
  const initial = fixture();
  const session = initial.sessionsById.s1;
  session.tasksById.t1.url = 'https://chatgpt.com/';
  session.tasksById.t1.normalizedUrl = 'https://chatgpt.com/';
  ...
  const result = await coordinator.submitWithDurableCheckpoint({
    ...
    submit: async () => ({
      ...
      normalizedObservedUrl: 'https://chatgpt.com/c/fresh-conversation-123',
    }),
  });

  assert.equal(result.status, InteractionResult.SUBMISSION_UNCERTAIN);
  assert.equal(result.normalizedObservedUrl, 'https://chatgpt.com/c/fresh-conversation-123');
  const state = await repo.load();
  const live = state.sessionsById.s1;
  assert.equal(live.operation.phase, OperationPhase.AMBIGUOUS);
  assert.equal(live.operation.targetUrl, 'https://chatgpt.com/c/fresh-conversation-123');
  assert.equal(live.tasksById.t1.normalizedUrl, 'https://chatgpt.com/c/fresh-conversation-123');
  assert.equal(live.tasksById.t1.lastConversationUrl, 'https://chatgpt.com/c/fresh-conversation-123');
});
