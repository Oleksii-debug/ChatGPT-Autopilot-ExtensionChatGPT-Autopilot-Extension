...
test('read-only UI commands do not reconcile runtime and broadcast STATUS_CHANGED back into their own reads', () => {
  ...
  assert.match(source, /if \(!READ_ONLY_UI_COMMANDS\.has\(message\.command\)\) \{[\s\S]*await reconcileRuntime\(\);[\s\S]*await reconcileAiReportAlarm\(\);[\s\S]*await reconcileAiManagerAlarm\(\);[\s\S]*\}/);
