...
test('CHECK_ONLY classifies a Chrome network error page and reloads it for bounded retry', async () => {
  let sends = 0;
  let injections = 0;
  let reloads = 0;
  const chromeApi = {
    ...
    scripting: {
      async executeScript() {
        injections += 1;
        throw new Error('Cannot access contents of url "chrome-error://chromewebdata/". Extension manifest must request permission to access this host.');
      },
    },
  };
  const transport = new ChromeInteractionTransport(chromeApi);
  ...
  await assert.rejects(
    () => transport.execute(17, {
      requestId: 'r-network-page',
      taskId: 't1',
      mode: 'CHECK_ONLY',
      expectedUrl: 'https://chatgpt.com/',
    }),
    (error) => error.safeDiagnosticCode === 'CHATGPT_PAGE_UNAVAILABLE'
