...
test('manifest wires the options UI and ChatGPT content scripts with bounded permissions', () => {
  ...
  assert.deepEqual(manifest.permissions.sort(), ['alarms', 'debugger', 'scripting', 'storage', 'tabs', 'unlimitedStorage']);
  assert.deepEqual(manifest.host_permissions, ['https://chatgpt.com/*', 'http://localhost/*', 'http://127.0.0.1/*', 'https://localhost/*', 'https://127.0.0.1/*', 'https://api.github.com/*']);
...
test('transient cold-start failure is retried once a wake event reaches the same live worker', async () => {
  ...
  globalThis.chrome = {
    ...
    alarms: {
      async clear(name) {
        if (name !== 'autopilot-core-wake') return true;
        clearAttempts += 1;
...
test('enabled cold wake reconciles durable submission state and coalesces simultaneous startup/alarm cycles', async () => {
  ...
  try {
    ...
    assert.equal(direct.outcomes[0].result.kind, 'UNCERTAIN_SETTLED_NO_RESEND');
    const settled = db[STORAGE_KEY].sessionsById.s1;
    assert.equal(settled.operation.phase, OperationPhase.FAILED_SAFE);
    assert.equal(settled.runState, RunState.RUNNING);
    ...
    assert.deepEqual(
      alarmCalls.filter(call => call[1] === 'autopilot-core-wake'),
