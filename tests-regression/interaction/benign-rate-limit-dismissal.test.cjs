...
test('exact Ukrainian notice is acknowledged and the SAME request continues in the SAME tab', async () => {
  let clicked = 0;
  ...
  const listener = install({
    ...
    async execute(request) {
      ...
      return { status: 'READY', requestId: request.requestId, taskId: request.taskId };
    },
  });

  const held = await send(listener, { requestId: 'op-1', taskId: 'task-1' });
  assert.equal(held.ok, true);
  assert.equal(held.data.status, 'READY');
  assert.equal(held.data.requestId, 'op-1');
  assert.equal(held.data.taskId, 'task-1');
  assert.equal(clicked, 1);
  assert.equal(adapterCalls, 1, 'the acknowledged request must continue immediately into the same adapter request');
  ...
  assert.equal(adapterCalls, 2, 'later Core-scheduled requests also continue normally');
});
...
test('Ukrainian rate-limit wording variant with Підтвердити is acknowledged safely', async () => {
  ...
  let adapterCalls = 0;
  let dialogVisible = true;
  const button = visibleElement('Підтвердити', { click() { clicked += 1; dialogVisible = false; } });
  const dialog = visibleElement(
  ...
  const document = {
    querySelectorAll(selector) {
      if (selector === '[role="dialog"], dialog, [role="alertdialog"], [aria-modal="true"]') return dialogVisible ? [dialog] : [];
      return [];
    },
  };
  ...
  assert.equal(clicked, 1);
  assert.equal(adapterCalls, 1, 'acknowledgement must continue the same interaction request');
});
