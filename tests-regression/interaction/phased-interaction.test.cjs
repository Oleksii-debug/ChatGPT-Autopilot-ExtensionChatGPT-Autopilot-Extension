...
test('INSERT_ONLY clears stale composer content and inserts the configured task prompt', async () => {
  const { adapter } = loadAdapter();
...
test('repeated copies of the same expected prompt are accepted before Send without forced rewriting', async () => {
  const { adapter } = loadAdapter();
  const prompt = 'exact pending prompt';
  const fx = fixture(prompt.repeat(3));
  fx.composer.select = () => {};
  fx.composer.setAttribute = () => {};
  fx.composer.removeAttribute = () => {};
  const result = await adapter.execute(request('INSERT_ONLY'), {
    document: fx.document,
    wait: async () => {},
    insert: async () => { fx.composer.value = prompt; },
  });
  assert.equal(result.status, adapter.STATUS.INSERTED_NOT_SENT);
  assert.equal(result.safeDiagnosticCode, 'INSERTION_REPEATED_PROMPT_ACCEPTED');
  assert.equal(fx.composer.value, prompt.repeat(3));
  assert.equal(fx.clicks(), 0);
});
