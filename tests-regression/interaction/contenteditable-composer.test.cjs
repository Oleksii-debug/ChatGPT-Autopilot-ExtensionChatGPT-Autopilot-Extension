...
function request(overrides = {}) {
  return Object.assign({
    requestId: 'contenteditable-op-1',
    taskId: 'task-1',
...
test('cancelled beforeinput fails closed instead of manufacturing insertion proof', async () => {
  const env = makeEnvironment({ execCommand: false });
  const composer = env.makeComposer({ directMutationUpdatesModel: true });
  const originalDispatch = composer.dispatchEvent;
  composer.dispatchEvent = (event) => {
    if (event.type === 'beforeinput') {
      env.events.push('beforeinput');
      return false;
    }
...
test('INSERT_ONLY clears stale ProseMirror content and inserts the configured task prompt', async () => {
  const env = makeEnvironment();
