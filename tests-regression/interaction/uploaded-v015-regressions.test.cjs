...
test('real chess 2925-char multiline prompt is proven after contenteditable reflow', async () => {
  const profile = JSON.parse(fs.readFileSync(path.join(ROOT, 'tests-regression', 'fixtures', 'Chess-profile.json'), 'utf8'));
...
test('MANUAL_REVIEW records the real diagnostic reason and pauses the Session', async () => {
  ...
  const session = {
    ...
    currentTaskIndex: 0,
    operation: { taskId: 't1', phase: schema.OperationPhase.INSERTING, updatedAt: 0 },
  };
...
test('explicit Resume clears only safe pre-submit manual review and retries', async () => {
  ...
  assert.equal(state.sessionsById.s1.tasksById.t1.status, 'RETRY_WAIT');
  assert.ok(state.sessionsById.s1.tasksById.t1.retryAfterAt > 100);
  assert.equal(state.sessionsById.s1.operation.phase, schema.OperationPhase.FAILED_SAFE);
});

test('Resume refuses to erase manual-policy review after a submit boundary was entered', async () => {
  const { pathToFileURL } = require('node:url');
  const commands = await import(pathToFileURL(path.join(ROOT, 'src/core/commands.js')).href);
  const protocol = await import(pathToFileURL(path.join(ROOT, 'src/shared/protocol.js')).href);
  const schema = await import(pathToFileURL(path.join(ROOT, 'src/core/schema.js')).href);
