...
test('fresh-launch durable recovery refuses ambiguous multi-turn history even when the prompt matches', async () => {
  const { adapter } = loadAdapter();
  const fx = fixture({ userMessages: ['older turn', 'exact recovery prompt'] });
  const result = await adapter.execute(request('VERIFY_AFTER_UNCERTAIN_SUBMIT', {
    recoveryLaunchUrl: 'https://chatgpt.com/'
  }), { document: fx.document });
  assert.equal(result.status, adapter.STATUS.SUBMISSION_UNCERTAIN);
...
test('non-fresh recovery never treats an unrelated active generation as proof of this Send', async () => {
  const { adapter } = loadAdapter();
  const fx = fixture({ buttons: [stopButton()] });
  const result = await adapter.execute(request('VERIFY_AFTER_UNCERTAIN_SUBMIT'), { document: fx.document });
  assert.equal(result.status, adapter.STATUS.SUBMISSION_UNCERTAIN);
  assert.notEqual(result.safeDiagnosticCode, 'RECOVERY_FRESH_GENERATION_STARTED');
  assert.equal(fx.clicks(), 0);
});
