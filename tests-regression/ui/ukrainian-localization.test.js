...
test('Ukrainian UI localization translates static labels and runtime states', () => {
  ...
  assert.equal(translateText('Task configuration mode'), 'Режим налаштування завдань');
  assert.equal(translateText('One ChatGPT link + one shared prompt'), 'Одне посилання ChatGPT + один спільний промпт');
  assert.equal(translateText('Number of tasks / cycles'), 'Кількість завдань / циклів');
...
test('Ukrainian UI localization covers portable configuration, bulk links, drafts and bounded log output', () => {
  ...
  assert.equal(translateText('Import and START ALL requested Sessions'), 'Імпортувати + ЗАПУСТИТИ ВСІ позначені сеанси');
  ...
  assert.equal(
    translateText('7 unique ChatGPT link(s) recognized; 5 new task(s) created. 2 link(s) did not fit the 1000-task limit.'),
    'Розпізнано унікальних посилань ChatGPT: 7; створено нових завдань: 5. Не вмістилося через обмеження у 1000 завдань: 2.'
  );
  assert.equal(
