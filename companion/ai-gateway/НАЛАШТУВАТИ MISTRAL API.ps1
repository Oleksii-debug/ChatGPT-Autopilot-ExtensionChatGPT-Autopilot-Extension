...
$keyFile = Join-Path $providerKeysDir 'MISTRAL_API_KEY.dpapi'
$presetTool = Join-Path $PSScriptRoot 'provider-presets.mjs'

New-Item -ItemType Directory -Path $providerKeysDir -Force | Out-Null
Write-Host 'Містраль буде додано як окремий сумісний постачальник через захищений локальний Gateway.'
...
$secure = Read-Host 'Введіть API key Містраль' -AsSecureString
...
& $nodeExe $presetTool '--apply-mistral' $configFile | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Не вдалося додати профіль Містраль до налаштувань Gateway.' }

Write-Host 'Профіль Містраль додано: захищене з’єднання, окремий ключ, окремий ідентифікатор постачальника.'
