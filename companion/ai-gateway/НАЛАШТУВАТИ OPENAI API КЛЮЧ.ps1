...
$keyFile = Join-Path $configDir 'openai-key.dpapi'
New-Item -ItemType Directory -Path $configDir -Force | Out-Null
Write-Host 'Ключ буде зашифровано Windows DPAPI для ПОТОЧНОГО Windows-користувача.'
Write-Host 'Chrome-розширення, JSON-профілі та діагностичні звіти цей ключ не отримують.'
$secure = Read-Host 'Введіть OpenAI API key' -AsSecureString
