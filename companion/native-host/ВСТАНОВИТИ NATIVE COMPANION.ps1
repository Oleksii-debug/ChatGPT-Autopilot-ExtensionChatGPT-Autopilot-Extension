...
$mcpPath = Join-Path $configDir 'mcp-commands.json'
if (-not (Test-Path -LiteralPath $mcpPath)) {
  @{
    schemaVersion = 1
    commands = @()
  } | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $mcpPath -Encoding UTF8
}
...
Write-Host 'MCP local-stdio за замовчуванням не запускає жодної команди: allowlist mcp-commands.json порожній.'
