...
function ensureBundledBootstrapApplied() {
  ...
  bootstrapBarrier = applyBundledBootstrapProfile({ repository: repo, chromeApi: chrome, profile: BUNDLED_BOOTSTRAP_PROFILE }).catch(error => {
...
function beginColdStartReconciliation() {
  ...
  })().catch(error => { coldStartBarrier = null; console.error('ChatGPT Autopilot cold-start reconciliation failed safely.'); throw error; });
...
async function dispatchPromptCadenceCommand(command, payload) {
  ...
  if (command === 'SET_PROMPT_CADENCE') {
    let config;
    const state = await repo.update(draft => { config = setPromptCadenceConfig(draft, payload.sessionId, payload.config || {}); return draft; });
...
async function dispatchBatchCommand(command, payload) {
  ...
  if (command === 'SET_BATCH_CHAT_FLOW') {
    let session;
    const state = await repo.update(draft => { session = configureBatchChatFlow(draft, payload.sessionId, payload.config || {}, Date.now()); return draft; });
    return batchChatFlowSnapshot(state, payload.sessionId);
  }
  if (command === 'START_BATCH_CHAT_FLOW') {
    let session;
    const state = await repo.update(draft => { session = startBatchChatFlow(draft, payload.sessionId, Date.now()); return draft; });
...
async function dispatchDriveCommand(command, payload) {
  ...
  if (command === 'SET_DRIVE_SOURCE') {
    let source;
    const state = await repo.update(draft => { const parsed = extractDriveFileId(payload.sourceUrl); source = setDriveSourceConfig(draft, payload.sessionId, { fileId: parsed.fileId, sourceUrl: payload.sourceUrl, target: payload.target }); return draft; });
  ...
  if (command === 'SYNC_DRIVE_SOURCE') {
    ...
    let acceptance;
    const state = await repo.update(draft => { acceptance = acceptDriveSnapshot(draft, payload.sessionId, snapshot); return draft; });
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  ...
  dispatchUiMessage(message).then(data => sendResponse({ ok: true, data })).catch(error => sendResponse({ ok: false, error: { message: error?.message || 'Core command failed' } }));
  return true;
});
