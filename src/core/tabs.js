...
function chatFlowNeedsNewChat(session, config) { const count = Number.isInteger(session.cadenceVerifiedSendCount) && session.cadenceVerifiedSendCount >= 0 ? session.cadenceVerifiedSendCount : 0; if (config.chatFlow.mode === 'new-chat-after') return count > 0 && count % config.chatFlow.newChatEveryN === 0; if (config.chatFlow.mode === 'staged') return count === 1 + config.chatFlow.continueCount; return false; }
async function bindChatFlowTaskTab(chromeApi, state, sessionId, task, session) {
  const config = getPromptCadenceConfig(state, sessionId); const key = task.id; const hint = state.tabHintsByTaskId[key]; const sessionHint = state.tabHintsByTaskId[chatFlowHintKey(sessionId)]; const shouldCreateNew = chatFlowNeedsNewChat(session, config);
  if (!shouldCreateNew) { const hinted = await getValidHintedTab(chromeApi, hint, { sessionId, kind: 'CHAT_FLOW' }) || await getValidHintedTab(chromeApi, sessionHint, { sessionId, kind: 'CHAT_FLOW' }); if (hinted) { const currentUrl = normalizedTabUrl(hinted) || CHAT_FLOW_ROOT_URL; task.url = currentUrl; task.normalizedUrl = currentUrl; const sharedHint = { tabId: hinted.id, sessionId, normalizedUrl: currentUrl, kind: 'CHAT_FLOW', boundAt: Date.now() }; state.tabHintsByTaskId[chatFlowHintKey(sessionId)] = { ...sharedHint }; state.tabHintsByTaskId[key] = { ...sharedHint }; return hinted; } }
  ...
  const currentUrl = normalizedTabUrl(tab) || CHAT_FLOW_ROOT_URL; task.url = currentUrl; task.normalizedUrl = currentUrl; const sharedHint = { tabId: tab.id, sessionId, normalizedUrl: currentUrl, kind: 'CHAT_FLOW', boundAt: Date.now() }; state.tabHintsByTaskId[chatFlowHintKey(sessionId)] = { ...sharedHint }; state.tabHintsByTaskId[key] = { ...sharedHint }; return tab;
}
