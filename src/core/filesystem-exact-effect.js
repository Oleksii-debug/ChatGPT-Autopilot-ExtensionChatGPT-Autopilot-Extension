...
import { FilesystemToolId } from './filesystem-agent-provider.js';
...
function assertProvider(provider) {
  if (!provider || typeof provider.invoke !== 'function' || typeof provider.authorize !== 'function') {
    throw new Error('Filesystem provider with side-effect-free authorization preflight is required');
  }
}
...
function observationFor(invocationId, result, observedAt) {
  return {
    ...
    invocationId,
    status: 'OK',
    summary: 'Filesystem Native Companion returned a bounded mutation result.',
 ...
 */
export class FilesystemExactEffectExecutorV1 {
  constructor({
    provider,
    store,
    verify,
    reconcileVerify = null,
    actorId = 'filesystem-exact-effect-executor',
    parentActorId = null,
    ...
    assertStore(store);
    if (typeof verify !== 'function') throw new Error('Independent filesystem effect verifier is required');
  ...
  async invoke({ invocation, policyDecision } = {}) {
    if (invocation?.toolId !== FilesystemToolId.WRITE_EXISTING_TEXT) {
    ...
    if (![ExactEffectPhase.PREPARED, ExactEffectPhase.SAFE_RETRY].includes(state.phase)) {
      ...
      error.code = state.phase === ExactEffectPhase.RECONCILE ? 'FILESYSTEM_RECONCILE_REQUIRED' : 'FILESYSTEM_EFFECT_NOT_EXECUTABLE';
    ...
    try {
      ...
      if (state.phase !== ExactEffectPhase.VERIFIED) {
        const error = new Error('Filesystem effect was not independently verified');
        error.code = state.phase === ExactEffectPhase.RECONCILE ? 'FILESYSTEM_RECONCILE_REQUIRED' : 'FILESYSTEM_EFFECT_VERIFICATION_FAILED';
        error.effectState = state;
        throw error;
      }
      ...
      const ambiguous = reduceExactEffectV1(state, {
        ...
        executionId: state.executionId,
        at: at(this.now),
        reasonCode: 'FILESYSTEM_EFFECT_UNCERTAIN',
        summary: 'Filesystem mutation did not reach a verified committed outcome; reconciliation is required before retry.',
      });
