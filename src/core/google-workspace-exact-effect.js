...
import { GoogleWorkspaceToolId } from './google-workspace-agent-provider.js';
...
function assertProvider(provider) {
  if (!provider || typeof provider.invoke !== 'function' || typeof provider.authorize !== 'function') {
    throw new Error('Google Workspace provider with side-effect-free authorization preflight is required');
  }
}
 ...
 */
export class GoogleWorkspaceExactEffectExecutorV1 {
  constructor({
    provider,
    store,
    verify,
    reconcileVerify = null,
    actorId = 'google-workspace-exact-effect-executor',
    parentActorId = null,
    ...
    assertStore(store);
    if (typeof verify !== 'function') throw new Error('Independent google-workspace effect verifier is required');
  ...
  async #admitExecution(invocation) {
    ...
    return this.#atomic(effectsById => {
      ...
      if (prior?.state && JSON.stringify(state.invocation) !== JSON.stringify(invocation)) {
        throw new Error('Google Workspace exact-effect invocation binding changed');
      }
  ...
  async #commitVerified(state) {
    if (state.phase !== ExactEffectPhase.VERIFIED) throw new Error('Only a verified google-workspace effect may be committed');
  ...
  #validateInitialVerification(state, raw, requestedAt, completedAt) {
    ...
    if ([this.actorId, this.parentActorId, state.invocation.providerId].filter(Boolean).includes(verifierId)) {
      throw new Error('Google Workspace verifier must be independent from the actor, parent controller, and effect provider');
    }
    if (requireId(verification.verificationAuthorityId, 'verificationAuthorityId') !== state.invocation.policyDecisionId) {
      throw new Error('Google Workspace verification authority must bind the admitted policy envelope');
    }
    if (requireId(verification.effectId, 'verification.effectId') !== state.effectId
      ...
      throw new Error('Google Workspace verification does not match the current exact-effect attempt');
    }
    if (verification.invocationId !== state.invocation.invocationId
      || verification.observationId !== state.observation.observationId) {
      throw new Error('Google Workspace verification does not match the effect invocation and observation');
    }
    ...
    if (verifiedAt < earliestFreshAt
      ...
      throw new Error('Google Workspace verification evidence is stale or has an invalid chronology');
    }
    return verification;
  }
  ...
  async invoke({ invocation, policyDecision } = {}) {
    ...
    if (admitted.status !== 'EXECUTING') {
      const error = new Error(state.phase === ExactEffectPhase.RECONCILE
        ? 'Google Workspace effect requires reconciliation before retry'
        : `Google Workspace effect cannot execute from ${state.phase}`);
      error.code = state.phase === ExactEffectPhase.RECONCILE ? 'GOOGLE_WORKSPACE_RECONCILE_REQUIRED' : 'GOOGLE_WORKSPACE_EFFECT_NOT_EXECUTABLE';
    ...
    try {
      ...
      if (state.phase !== ExactEffectPhase.VERIFIED) {
        const error = new Error('Google Workspace effect was not independently verified');
        error.code = state.phase === ExactEffectPhase.RECONCILE ? 'GOOGLE_WORKSPACE_RECONCILE_REQUIRED' : 'GOOGLE_WORKSPACE_EFFECT_VERIFICATION_FAILED';
        error.effectState = state;
        throw error;
      }
      ...
      const ambiguous = reduceExactEffectV1(state, {
        ...
        executionId: state.executionId,
        at: at(this.now),
        reasonCode: 'GOOGLE_WORKSPACE_EFFECT_UNCERTAIN',
        summary: 'Google Workspace mutation did not reach a verified committed outcome; reconciliation is required before retry.',
      });
  ...
  async reconcile(request = {}) {
    exactKeys(request, RECONCILE_REQUEST_KEYS, 'Google Workspace reconciliation request');
    ...
    // no google-workspace I/O and its canonical event/commit ids are deterministic.
    if (state.phase === ExactEffectPhase.VERIFIED
    ...
    if (state.phase !== ExactEffectPhase.RECONCILE) throw new Error('Google Workspace effect is not awaiting reconciliation');
