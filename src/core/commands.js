...
export class CoreCommandDispatcher {
  ...
  async execute(command, payload = {}) {
    ...
    if (command === CoreCommand.DUPLICATE_SESSION) {
      const state=await this.repo.update(d=>{ const old=requireSession(d,payload.sessionId); const now=this.now(); const copy=structuredClone(old); copy.id=crypto.randomUUID(); copy.version=1; copy.name=`${old.name} copy`; copy.runState=RunState.STOPPED; copy.currentTaskIndex=0; copy.operation=null; copy.nextAllowedSendAt=0; copy.lastActionAt=0; copy.lastSuccessfulSendAt=0; copy.lastError=''; copy.onePassCompletedTaskIds=[]; copy.createdAt=now; copy.updatedAt=now; copy.pausedByMaster=false; const nextTasks={}; copy.taskOrder=old.taskOrder.map(id=>{const nid=crypto.randomUUID(); nextTasks[nid]={...structuredClone(old.tasksById[id]),id:nid,status:'IDLE',lastCheckedAt:0,lastVerifiedSendAt:0,lastVerifiedFingerprint:'',retryAfterAt:0,manualReviewReason:''}; return nid;}); copy.tasksById=nextTasks; d.sessionsById[copy.id]=copy; d.sessionOrder.push(copy.id); appendLog(d,copy.id,'Session duplicated',{at:now}); return d;}); const id=state.sessionOrder.at(-1); return {session:sessionToUi(state.sessionsById[id],state)};
    }
