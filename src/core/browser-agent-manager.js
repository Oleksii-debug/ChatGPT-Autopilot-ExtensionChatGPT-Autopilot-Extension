...
export class BrowserAgentManager {
  ...
  async claimSpecialistHandoffsAcrossJobs({ maxConcurrentHandoffs, ...payload } = {}) {
    ...
    await this.update(store => {
      ...
      const capacityOwnerships = store.order.flatMap(jobId => store.byId[jobId]?.runtime?.specialistExecutionOwnerships || [])
        .map(normalizeExecutionOwnershipV1)
        .filter(item => SPECIALIST_CAPACITY_STATES.has(item.state));
      const effectIds = new Set();
      for (const ownership of capacityOwnerships) {
