...
test('accepted navigation with stale first readback remains fenced until fresh reconciliation', async () => {
  ...
  const chromeApi = {
    ...
    tabs: {
      async get(id) { return { id, url: settled ? newUrl : oldUrl, status: settled ? 'complete' : 'loading' }; },
      async update(id, update) {
        updates.push([id, structuredClone(update)]);
        return { id, ...update, status: 'loading' };
      },
    },
    scripting: {
      async executeScript() {
        return [{ result: {
          url: settled ? newUrl : oldUrl,
          readyState: settled ? 'complete' : 'loading',
          visibleSelectors: [],
        } }];
      },
    },
  ...
  const provider = createChromeDeterministicWebProviderV1({
    ...
    now: () => at,
    leaseId: () => 'navigation-lease',
  });

  const first = await provider.invoke({
    ...auth('nav-1'),
    targetId: 'tab:7',
    action: { kind: 'NAVIGATE', url: newUrl },
    postcondition: { url: newUrl },
  });
  assert.equal(first.status, 'AMBIGUOUS');
  assert.equal(first.reconcileRequired, true);
  assert.equal(first.verification.reasonCode, 'URL_MISMATCH');
  assert.equal(first.effectState.phase, 'RECONCILE');
  assert.equal(updates.length, 1);
  assert.equal(storage['autopilot.deterministicWebRuntime.v1'].leasesByTargetId['tab:7'].ownerInvocationId, 'nav-1');

  const competing = await provider.invoke({
    ...auth('nav-2'),
    targetId: 'tab:7',
    action: { kind: 'NAVIGATE', url: 'https://example.test/other' },
    postcondition: { url: 'https://example.test/other' },
  });
  assert.equal(competing.status, 'TARGET_CONFLICT');
  assert.equal(updates.length, 1, 'competing navigation must not dispatch while first navigation is unresolved');

  settled = true;
  const reconciled = await provider.reconcile({ invocationId: 'nav-1', outcome: 'VERIFIED' });
  assert.equal(reconciled.phase, 'COMMITTED');
  assert.equal(storage['autopilot.deterministicWebRuntime.v1'].leasesByTargetId['tab:7'], undefined);
});
