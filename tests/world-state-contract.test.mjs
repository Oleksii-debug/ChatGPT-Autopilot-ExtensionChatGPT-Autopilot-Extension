...
test('WorldStateObservationV1 binds exact resource revision, bytes, time and evidence', () => {
  ...
  assert.throws(
    () => normalizeWorldStateObservationV1(observation({ observedAt: '2026-09-24T22:40:00Z' })),
...
test('strict boundary rejects accessors, symbols, hidden fields, exotic prototypes, sparse arrays and coercion aliases', () => {
  ...
  let lengthReads = 0;
  const proxiedObservations = new Proxy([observation()], {
    get(target, key, receiver) {
      if (key === 'length') lengthReads += 1;
      return Reflect.get(target, key, receiver);
    },
  });
  const normalizedProxySnapshot = normalizeWorldStateSnapshotV1(snapshot({
    observations: proxiedObservations,
  }));
  assert.equal(normalizedProxySnapshot.observations.length, 1);
