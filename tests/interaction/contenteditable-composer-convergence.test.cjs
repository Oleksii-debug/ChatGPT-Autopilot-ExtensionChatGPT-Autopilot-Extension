...
function loadAdapter(extra = {}) {
  ...
  const sandbox = Object.assign({
    ...
    Event: FakeEvent, InputEvent: FakeEvent,
...
function request(overrides = {}) {
  return Object.assign({ requestId: 'op1', taskId: 't1', expectedUrl: 'https://chatgpt.com/c/abc', promptText: 'Exact ProseMirror prompt ✅', mode: 'INSERT_ONLY' }, overrides);
}
...
function makeEnvironment(options = {}) {
  ...
  const defaultView = { Event: FakeEvent, InputEvent: FakeEvent, getSelection() { return { removeAllRanges() {}, addRange() {} }; } };
  const ownerDocument = {
    defaultView,
  ...
  function makeComposer({ initialText = '', directMutationUpdatesModel = false } = {}) {
    ...
    const element = {
      id: 'prompt-textarea', isConnected: true, hidden: false, disabled: false, tagName: 'DIV', ownerDocument,
      parentElement: { querySelectorAll() { return []; } },
      get innerText() { return frameworkText; }, get textContent() { return domText; },
      set textContent(value) { domText = String(value); if (directMutationUpdatesModel) frameworkText = String(value); },
      getAttribute(name) { if (name === 'contenteditable') return 'true'; if (name === 'id') return 'prompt-textarea'; if (name === 'aria-disabled') return 'false'; if (name === 'role') return 'textbox'; return null; },
      getBoundingClientRect() { return { width: 500, height: 48 }; },
      closest() { return null; },
      focus() { state.activeElement = element; },
      dispatchEvent(event) { events.push(event.type); return true; },
      _setFrameworkText(value) { frameworkText = String(value); domText = String(value); events.push('execCommand:insertText'); }
    };
    return element;
  }
  const send = { isConnected: true, hidden: false, disabled: false, tagName: 'BUTTON', innerText: '', getAttribute(name) { if (name === 'aria-label') return 'Send'; if (name === 'aria-disabled') return 'false'; return null; }, getBoundingClientRect() { return { width: 24, height: 24 }; }, click() { clicks += 1; } };
  function buildDocument(composer) {
    return { body: { innerText: '' }, querySelectorAll(selector) {
...
test('INSERT_ONLY uses ownerDocument editing transaction and never Sends', async () => {
  const env = makeEnvironment();
  const composer = env.makeComposer();
  const adapter = loadAdapter({ document: { queryCommandSupported() { return false; }, execCommand() { throw new Error('wrong doc'); } } });
  const result = await adapter.execute(request(), { document: env.buildDocument(composer), wait: async () => {} });
...
test('contenteditable fallback is proof only when resulting text is observable', async () => {
...
test('existing contenteditable draft is never overwritten', async () => {
  const env = makeEnvironment();
