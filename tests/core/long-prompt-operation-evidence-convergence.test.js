...
test('executor accepts explicit operation-bound accepted-representation evidence', async () => {
...
test('attachment-like status without operation-bound evidence still fails closed', async () => {
  const repo = new Repo(state());
  const transport = { async execute(_tab, request) {
    if (request.mode === 'CHECK_ONLY') return { status: InteractionResult.READY };
    return {
