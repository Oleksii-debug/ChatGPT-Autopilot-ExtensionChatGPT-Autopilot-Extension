...
test('native insertion uses durable prompt text and cannot accept a forged prompt',async()=>{
 const f=setup('insert');f.message.promptText='injected different prompt';await f.run();assert.equal(f.calls[1].method,'Input.insertText');assert.equal(f.calls[1].args.text,'canonical prompt');
});
