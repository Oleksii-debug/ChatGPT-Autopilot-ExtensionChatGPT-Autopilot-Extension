  ...
  return {
    ...
    chrome: {
      ...
      tabs: {
        async get(id) {
          tabGets += 1;
          if (closeBeforeReconcile && tabGets > 1) throw new Error('tab closed');
          return { id, url: 'https://example.test/start', status: 'complete' };
        },
        async update(id, update) { return { id, ...update }; },
      },
      scripting: {
        async executeScript(request) {
          if ((request.args || [])[0] === 'CLICK') {
            clickAttempts += 1;
            if (clickAttempts === 1) throw new Error('connection lost after click dispatch');
            return [{ result: { ok: true } }];
          }
          return [{ result: {
            url: 'https://example.test/start',
            readyState: 'complete',
            visibleSelectors: ['#ready'],
          } }];
        },
      },
    },
...
test('fresh independent Chrome readback can reconcile an ambiguous exact effect to committed without replay', async () => {
  ...
  assert.equal(fixture.storage['autopilot.deterministicWebRuntime.v1'].leasesByTargetId['tab:7'], undefined);
});
